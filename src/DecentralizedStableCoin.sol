...
contract DecentralizedStableCoin is ERC20Burnable, Ownable {
    error DecentralizedStableCoin__BurnAmountZero();
    error DecentralizedStableCoin__BurnAmountExceedsBalance();
    error DecentralizedStableCoin__MintToZeroAddress();
    error DecentralizedStableCoin__MintAmountZero();
    ...
    function burn(uint256 _amount) public override onlyOwner {
        ...
        if (_amount < 0) {
            revert DecentralizedStableCoin__BurnAmountZero();
        }
    ...
    function mint(address _to, uint256 _amount) external onlyOwner returns (bool) {
        ...
        if (_amount <= 0) {
            revert DecentralizedStableCoin__MintAmountZero();
        }
        _mint(_to, _amount);
        return true;
    }
